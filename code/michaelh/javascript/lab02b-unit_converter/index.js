...
// let units_org = prompt('What are the units of the distance?')
// let distance_units_org = prompt(`What is the distance in ${units_org}?`)
// let units_final = prompt('What are the units to convert to? ')

let data_input = document.querySelector('#data_input');
let units_final_input = document.querySelector('#units_final_input');
...
run_bt.onclick = function() {
    ...
    data = data.split(" ");
    let units_org = data[1];
    let distance_units_org = data[0];    
    let units_final = units_final_input.value;

    let distance_m = parseFloat(distance_units_org);
    if (units_org == 'ft'){
        distance_m *= 0.3048;
    } else if (units_org == 'km'){
        distance_m *= 1000;
    } else if (units_org == 'mi'){
        distance_m *= 1609.34;
    } else if (units_org == 'yd') {
        distance_m *= 0.9144;
    } else if (units_org == 'in') {
        distance_m *= 0.0254;    
    }

    let distance_final = distance_m;
    if (units_final == 'ft'){
        distance_final /= 0.3048;
    } else if (units_final == 'km'){
        distance_final /= 1000;
    } else if (units_final == 'mi'){
        distance_final /= 1609.34;
    } else if (units_final == 'yd') {
        distance_final /= 0.9144;
    } else if (units_final == 'in') {
        distance_final /= 0.0254;    
    }

    output_div.innerText = `${distance_units_org} ${units_org} is ${distance_final} ${units_final}`;
}